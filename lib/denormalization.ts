...
export async function updateQuoteCounts(quoteId: string | any): Promise<void> {
  try {
    ...
    await quotesCollection.updateOne({ _id: quoteObjId } as any, {
      $set: {
...
export async function updateAllQuoteCounts(): Promise<{ updated: number; duration: number }> {
  ...
  try {
    ...
    // Create lookup maps
    const likeMap = new Map(likeCounts.map((l: any) => [normalizeId(l._id), l.count]));
    const dislikeMap = new Map(dislikeCounts.map((d: any) => [normalizeId(d._id), d.count]));

    // Get all quote IDs
    const quotes = await quotesCollection.find({}).project({ _id: 1 }).toArray();
