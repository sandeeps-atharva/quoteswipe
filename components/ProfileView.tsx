  ...
  const handleSaveName = useCallback(async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setProfileData((prev) =>
          prev ? { ...prev, user: { ...prev.user, name: data.user.name } } : null
        );
    ...
    const content = (
      <>
        ...
        >
          <Icon
            size={18}
            className={`sm:w-5 sm:h-5 ${danger ? 'text-red-500' : gradient ? 'text-white' : 'text-gray-600 dark:text-gray-400'}`}
          />
        </div>
        <div className="flex-1 min-w-0">
          <p
            className={`font-medium text-sm sm:text-base ${danger ? 'text-red-600 dark:text-red-400' : 'text-gray-900 dark:text-white'}`}
          >
            {title}
          </p>
          {subtitle && <p className="text-[10px] sm:text-xs text-gray-500 truncate">{subtitle}</p>}
        </div>
      ...
      <header className="sticky top-0 z-10 bg-white/80 dark:bg-stone-900/80 backdrop-blur-xl border-b border-stone-200/50 dark:border-stone-800/50">
        <div className="max-w-2xl mx-auto px-3 sm:px-4 md:px-6 py-2.5 sm:py-3">
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            <h1 className="text-lg sm:text-xl font-bold text-stone-900 dark:text-white">Profile</h1>
          </div>
        </div>
      </header>
            ...
            <div className="space-y-4 sm:space-y-6">
                ...
                <div className="relative pt-20 sm:pt-24 px-4 sm:px-6 pb-5 sm:pb-6">
                  ...
                  <div className="absolute -top-0 left-1/2 -translate-x-1/2 translate-y-8 sm:translate-y-10">
                    <div className="relative">
                      ...
                      {/* Auth Provider Badge */}
                      <div
                        className={`absolute -bottom-1 -right-1 w-8 h-8 sm:w-9 sm:h-9 rounded-xl flex items-center justify-center shadow-lg ${
                          profileData.user.auth_provider === 'google' ? 'bg-white' : 'bg-amber-500'
                        }`}
                      >
                    ...
                    >
                      ...
                      >
                        <div className="p-4 border-b border-stone-100 dark:border-stone-800">
                          <h3 className="text-lg font-semibold text-center text-stone-900 dark:text-white">
                            Profile Picture
                          </h3>
                        </div>
                        <div className="p-2">
              ...
              <div className="grid grid-cols-4 gap-2 sm:gap-3">
                {[
                  ...
                  <div
                    key={idx}
                    className={`${stat.bg} rounded-xl sm:rounded-2xl p-3 sm:p-4 text-center relative overflow-hidden group`}
                  >
                    <div
                    ...
                    <stat.icon
                      size={18}
                      className={`sm:w-5 sm:h-5 mx-auto mb-1 bg-gradient-to-br ${stat.color} text-transparent bg-clip-text`}
                      fill="currentColor"
                      style={{ WebkitTextStroke: '0.5px', stroke: 'currentColor' }}
                    />
                    <div
                      className={`text-xl sm:text-2xl font-bold bg-gradient-to-br ${stat.color} bg-clip-text text-transparent`}
                    >
                      {stat.count}
                    </div>
                ...
                <div className="p-1">
                  {activeTab === 'saved' ? (
                      ...
                      <div className="grid grid-cols-3 gap-0.5 sm:gap-1">
                        {savedQuotes.map((quote) => {
                          ...
                          const bgStyle = quote.custom_background
                            ? quote.custom_background.startsWith('linear-gradient') ||
                              quote.custom_background.startsWith('radial-gradient')
                              ? { background: quote.custom_background }
                              : {
                                  backgroundImage: `url(${quote.custom_background})`,
                                  backgroundSize: 'cover',
                                  backgroundPosition: 'center',
                                }
                            : (() => {
                                const randomBg = getRandomBackgroundForQuote(quote.id);
                                return randomBg.url?.startsWith('linear-gradient') ||
                                  randomBg.url?.startsWith('radial-gradient')
                                  ? { background: randomBg.url }
                                  : {
                                      backgroundImage: `url(${randomBg.url})`,
                                      backgroundSize: 'cover',
                                      backgroundPosition: 'center',
                                    };
                              })();
                              ...
                              <div className="absolute inset-0 p-2 flex flex-col justify-between">
                                ...
                                <div className="text-left">
                                  <p className="text-white text-[9px] sm:text-[11px] font-medium leading-tight line-clamp-3 drop-shadow-lg">
                                    &ldquo;{quote.text.slice(0, 80)}
                                    {quote.text.length > 80 ? '...' : ''}&rdquo;
                                  </p>
                    ...
                    <div className="py-12 px-4 text-center">
                      ...
                      <p className="text-sm text-stone-500 mb-4">
                        Create your first inspiring quote
                      </p>
                      <button
                  ...
                  <div className="flex-1 text-left">
                    ...
                    <div className="flex items-center gap-3 mt-1.5">
                      {[
                        ...
                        <span
                          key={idx}
                          className="flex items-center gap-1 text-white/80 text-[10px] sm:text-xs"
                        >
